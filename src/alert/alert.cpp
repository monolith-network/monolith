...
alert_manager_c::alert_manager_c(alert_manager_c::configuration_c config)
    : _config(config) {}

void alert_manager_c::trigger(const int id, const std::string message) {

  LOG(INFO) << TAG("alert_manager_c::trigger") << "Alert ID: " << id << " => "
            << message << "\n";
    ...
    if (it != _send_map.end()) {
      ...
      if (diff.count() <= _config.alert_cooldown_seconds) {
        LOG(INFO) << TAG("alert_manager_c::trigger")
                  << "Actively limiting alert for : " << id
                  << ". Seconds left on limiter: "
                  << _config.alert_cooldown_seconds - diff.count() << "s \n";
        return;
      }
  ...
  if (_config.max_alert_sends != 0) {
    if (_total_alerts_sent++ >= _config.max_alert_sends) {
      LOG(INFO) << TAG("alert_manager_c::trigger")
                << "Maximum number of alerts (" << _config.max_alert_sends
                << ") has been reached\n";
      return;
    }
  }
  ...
  {
    const std::lock_guard<std::mutex> lock(_send_map_mutex);
    _send_map[id] = {.last_send = std::chrono::steady_clock::now(),
                     .num_sends = num_sends};
  }
}
