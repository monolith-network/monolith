...
inline std::u16string utf8_to_ucs2(std::string const &input) {
...
inline std::string ucs2_to_utf8(std::u16string const &input) {
   std::wstring_convert<std::codecvt_utf8<char16_t>, char16_t> convert;
...
size_t twilio_c::_null_write(char *ptr, size_t size, size_t nmemb,
                             void *userdata) {
   return size * nmemb;
}
...
size_t twilio_c::_stream_write(char *ptr, size_t size, size_t nmemb,
                               void *userdata) {
...
bool twilio_c::setup() {
   ...
   if (_config.account_id.empty()) {
      LOG(ERROR) << TAG("twilio_c::send_message")
                 << "Twilio account id not set\n";
      return false;
   }
...
bool twilio_c::send_message(std::string message) {
   ...
   char *message_escaped = curl_easy_escape(curl, message.c_str(), 0);

   std::stringstream url;
   std::string url_string;
   url << "https://api.twilio.com/2010-04-01/Accounts/" << _config.account_id
       << "/Messages";
   url_string = url.str();
