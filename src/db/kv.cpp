...
kv_c::kv_c(const std::string &db_location) : _db_location(db_location) {}
...
bool kv_c::ensure_open() {
   ...
   LOG(INFO) << TAG("kv_c::ensure_open")
             << "Attempting to open : " << _db_location << "\n";
   ...
   if (!status.ok()) {
      LOG(ERROR) << TAG("kv_c::ensure_open")
                 << "Unable to open database file : " << _db_location << "\n";
      return false;
   }
