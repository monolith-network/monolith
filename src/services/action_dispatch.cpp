...
void action_dispatch_c::burst() {
   ...
   for(auto& action_entry : selected_actions) {
      ...
      crate::networking::message_writer_c writer(action_entry.address, action_entry.port);
      ...
      while(!sent && write_okay && attempts++ < MAX_RETRIES) {
         ...
         if ((writer.write(encoded_action, write_okay) == encoded_action.length())) {
            sent = true;
         }
      }
...
bool action_dispatch_c::dispatch(std::string controller_id,
   ...
   LOG(TRACE) << TAG("action_dispatch_c::dispatch")
               << controller_id << " | "
               << action_id << " | "
               << value << "\n";
   ...
   if (!data.has_value()) {
      LOG(FATAL) << TAG("action_dispatch_c::dispatch")
                  << "Given controller id is not regestered: "
                  << controller_id
                  << "\n";
      return false;
   }
   ...
   LOG(FATAL) << TAG("action_dispatch_c::dispatch")
               << "Failed to locate action id ["
               << action_id << "] on controller ["
               << controller_id << "]\n";
   return false;
}
