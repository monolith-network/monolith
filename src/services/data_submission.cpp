...
void data_submission_c::submit_metrics() {
  ...
  */
  ...
  for (auto &entry : metrics) {
    ...
    //       crate::registrar::cache and we could implement something like that
    //       in th future but we'd want to do that in a shared place so others
    //       can use a cache mechanism like that - maybe we can update crate to
    //       have a general cache like that
    ...
    if (!_stream_server->submit_metric(entry.metric)) {
      ...
      // Check to see if the submission attempts indicate that we need to drop
      // the thing
      ...
      // medium until we iterate through this entire burst. Once this burst is
      // complete we will take the lock from the queue and re-enqueue them
