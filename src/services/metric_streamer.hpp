...
private:
  /*
     ...
     Similarly, we need to make sure that metrics being submitted aren't keeping
     our mutexes locked for too long so we schedule BURST_STREAM_METRIC number
     of metrics to be burst out to destinations
  */
  ...
  // be added/removed too quickly though if that were to change we would need to
  // update this to something that would take that into consideration
  //
  struct endpoint {
    std::string address;
    uint32_t port{0};
  ...
  // together. Consideration of having two seperate queues, one for add, one for
  // delete, was had but deemed pointless. Here we stuff them together and
